...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, MatTableModule, MatChipsModule, MatTooltipModule, MatIconModule, ElliottChartThumbnailComponent],
  template: `
        ...
        <table mat-table [dataSource]="sortedResults()" class="results-table mat-elevation-z1">
          ...
          <ng-container matColumnDef="name">
            ...
            <td
              ...
              *matCellDef="let row"
              class="col-name"
              [matTooltip]="row.name ? '' : 'Kein Name vom Datenanbieter geliefert – zeige Ticker'"
            >
              {{ row.name ?? row.ticker }}
            </td>
          </ng-container>
          ...
          <ng-container matColumnDef="interval">
            <th mat-header-cell *matHeaderCellDef class="col-center">TF</th>
          ...
          <ng-container matColumnDef="price">
            ...
            <td mat-cell *matCellDef="let row" class="col-right numeric">
              {{ row.current_price != null ? currencySymbol(row.currency, row.ticker) + ' ' + row.current_price.toFixed(2) : '–' }}
            </td>
          </ng-container>
          ...
          <ng-container matColumnDef="elliott">
            ...
            <td mat-cell *matCellDef="let row" class="col-center">
              @if (elliottWaveLabel(row); as label) {
                <span
                  [class]="elliottWaveBadgeClass()"
                  [matTooltip]="
                    row.elliott_wave
                      ? 'Muster vollständig erkannt (Konfidenz ausreichend)'
                      : 'Zwischenstand – Muster (noch) nicht sicher/vollständig genug'
                  "
                  >{{ label }}</span
                >
              }
          ...
          <!-- Elliott-Wave-Chart (Thumbnail, Klick öffnet Modal) -->
          <ng-container matColumnDef="chart">
            <th mat-header-cell *matHeaderCellDef class="col-center">Chart</th>
            <td mat-cell *matCellDef="let row" class="col-center">
              <app-elliott-chart-thumbnail
                [data]="row.elliott_chart"
                (opened)="openChartModal(row)"
              ></app-elliott-chart-thumbnail>
            </td>
          </ng-container>
    ...
    <!-- Legende -->
    <div class="legend">
      <span class="badge-true">True</span> Kriterium erfüllt &nbsp;·&nbsp; <span class="badge-false">False</span> Nicht erfüllt
      &nbsp;·&nbsp; <span class="score score-3">3/3</span> Stärkstes Signal &nbsp;·&nbsp;
      <span class="score score-2">2/3</span> Mittleres Signal &nbsp;·&nbsp; <span class="ml-badge ml-strong">75% 🔥</span> KI:
      Starkes Umkehrsignal &nbsp;·&nbsp; <span class="ml-badge ml-moderate">55% ↑</span> KI: Mittleres Signal
    </div>
  `,
  styles: [
    `
...
export class ResultsTableComponent {
  ...
  readonly sortedResults = computed(() => {
    ...
    return data.sort((a, b) => {
      ...
      switch (col) {
        case 'ticker':
          return f * a.ticker.localeCompare(b.ticker);
        case 'name':
          return f * (a.name ?? '').localeCompare(b.name ?? '');
        case 'price':
          return f * ((a.current_price ?? -Infinity) - (b.current_price ?? -Infinity));
        case 'trend':
          return f * ((a.trend_pct ?? -Infinity) - (b.trend_pct ?? -Infinity));
        case 'elliott':
          return f * (Number(a.elliott_wave) - Number(b.elliott_wave));
        case 'stochastic':
          return f * (Number(a.stochastic) - Number(b.stochastic));
        case 'macd':
          return f * (Number(a.macd_histogram) - Number(b.macd_histogram));
        case 'score':
          return f * (a.criteria_met - b.criteria_met);
        case 'direction':
  ...
  /** Re-Export fürs Template (Angular-Templates rufen keine importierten Funktionen direkt auf). */
   ...
   * z.B. "A-B-" wenn Korrektur-Welle C im Entstehen ist) - zeigt so auch
   * unvollständige/unsichere Muster, nicht nur vollständig bestätigte.
  ...
  /** Badge-Klasse: einheitliches Styling, Konfidenz nur noch per Tooltip erkennbar. */
  elliottWaveBadgeClass(): string {
    return 'elliott-badge';
  }
  ...
  openChartModal(row: StockResult): void {
    if (!row.elliott_chart) {
      return;
    }
    this.dialog.open(ElliottChartModalComponent, {
      data: {
        ticker: row.ticker,
        name: row.name,
        stage: row.elliott_wave_stage,
        chart: row.elliott_chart,
      },
      width: '760px',
      maxWidth: '90vw',
    });
